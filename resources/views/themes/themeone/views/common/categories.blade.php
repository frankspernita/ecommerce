     ...
     @foreach($result['commonContent']['categories'] as $categories_data)
      ...
      <li class="nav-item dropdown">
        <a href="{{ URL::to('/shop')}}?category={{$categories_data->slug}}" class="nav-link dropdown-toggle">
          <i class="fa fa-angle-double-right" aria-hidden="true"></i>{{$categories_data->name}} @if(count($categories_data->sub_categories)>0) <i class="fa fa-angle-right " aria-hidden="true"></i> @endif
        </a>
